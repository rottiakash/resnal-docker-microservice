...
class genXLDash(APIView):
    def get(self,request):
        ...
        border_format = workbook.add_format({'border':1})
        border_format_fcd_green = workbook.add_format({'align': 'center','border':1,'bg_color':'green'})
        border_format_fcd_blue = workbook.add_format({'align': 'center','border':1,'bg_color':'blue'})
        border_format_fcd_yellow = workbook.add_format({'align': 'center','border':1,'bg_color':'yellow'})
        border_format_fcd_purple= workbook.add_format({'align': 'center','border':1,'bg_color':'purple'})
        border_format_fcd_red = workbook.add_format({'align': 'center','border':1,'bg_color':'red'})
        j = 1
        for i in results:
            if i.totalFCD=="FCD":
                fcd_format = border_format_fcd_green
        ...
        chart = workbook.add_chart({'type': 'column'})
        data = ["FCD","FC","SC","P"]
        chart.add_series({'data_labels': {'value': True, 'position':'inside_end'},'categories': '=Sheet1!$O$4:$R$4','values': '=Sheet1!$O$5:$R$5'})
        chart.set_legend({'none': True})
        worksheet.insert_chart('O9', chart)
    ...
    'points': [
        ...
        with open('/app/Export.xlsx', 'rb') as fh:
            response = HttpResponse(fh.read(), content_type="application/vnd.ms-excel")
            response['ContentDisposition'] = 'inline; filename=' + os.path.basename('/app/Export.xlsx')
            return response
